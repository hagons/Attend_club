module.exports = function (app) {
  ...
  route.get('/:name', (req, res) => {
    var name = urlencode.decode(req.params.name);
    var isexist = 'select people_id from mac_member where name=?';
    var counting = 'select check_id, out_time from mac_list where in_time > curdate() and ?';
  ...
  route.get(['/my/:people_id', '/my/:people_id/:status'], (req, res) => {
    ...
    var sql = 'select m.name, l.check_id, DATE_FORMAT(l.in_time,"%Y%m%d%H%i%s") as in_time, DATE_FORMAT(l.in_time_edit,"%Y%m%d%H%i%s") as in_time_edit, l.in_co, DATE_FORMAT(l.out_time,"%Y%m%d%H%i%s") as out_time, DATE_FORMAT(l.out_time_edit,"%Y%m%d%H%i%s") as out_time_edit, l.out_co from mac_list as l, mac_member as m where m.people_id = l.people_id and m.people_id = ? and l.in_time > curdate() order by l.in_time asc';
