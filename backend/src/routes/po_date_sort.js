module.exports = function(app) {
  ...
  route.post('/graph', (req, res) => {
    ...
    var sql =
      'select m.name, count(l.people_id) as count from po_list as l right outer join po_member as m on l.people_id = m.people_id and l.in_time like ? group by m.name order by count desc, l.in_time desc';
  ...
  route.post('/row_data', (req, res) => {
    ...
    var time = getTime.SQLMonth(month);
    var sql =
      'select l.check_id, m.name, DATE_FORMAT(l.in_time,"%Y%m%d%H%i%s") as in_time, dATE_FORMAT(l.in_time_edit,"%Y%m%d%H%i%s") as in_time_edit, l.in_co, DATE_FORMAT(l.out_time,"%Y%m%d%H%i%s") as out_time, DATE_FORMAT(l.out_time_edit,"%Y%m%d%H%i%s") as out_time_edit, l.out_co from po_list as l, po_member as m where m.people_id = l.people_id and l.in_time like ? order by l.in_time DESC,m.name';
    conn.query(sql, time, function(err, rows, fields) {
      if (err) {
        console.log(getTime.Date(), err);
        res.status(500).send('Internal Server Error');
  ...
  route.post('/not_today', (req, res) => {
    var sql =
      'select name from po_member where people_id not in (select people_id from po_list where in_time>curdate() group by people_id)';
  ...
  route.post('/excel', (req, res) => {
    ...
    var sql =
      'SELECT m.name, l.in_time, l.in_time_edit, l.in_co, l.out_time, l.out_time_edit, l.out_co FROM po_list as l, po_member as m where l.people_id=m.people_id and l.in_time like ? into outfile ? CHARACTER SET euckr';
